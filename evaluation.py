...
for name in image_names_we_got:
    ...
    print("Number of boxes detected:", len(all_boxes))
    #cv2.imshow("final", img)
    #cv2.imshow("final_supression", img_sup)
    #cv2.waitKey(0)
...
############## EVALUATION PARAMETERS ##############################
    ...
    x_cord = [(x_min[i] + x_max[i])/2 for i in range(len(x_min))]
    y_cord = [(y_min[i] + y_max[i])/2 for i in range(len(y_min))]
    ...
    # ###### FIRST PARAMETER(Alignment) ######

    x_norm,x_m,x_sd = normalize(x_cord)
    y_norm,y_m,y_sd = normalize(y_cord)
    linear_regressor = LinearRegression()
    linear_regressor.fit(y_norm, x_norm)
    X_pred = linear_regressor.predict(y_norm)

    d = X_pred - x_norm
    d = d**2
    cost = np.sum(d)
    cost = cost/(2*x_norm.shape[0])
    alignment.append(cost)
    print("Parameter 1 score(Alignment): ",cost)
    plt.scatter(y_norm, x_norm)
    plt.xlabel('image height')
    plt.ylabel('image width')
    plt.plot(y_norm, X_pred, color='red')
    plt.savefig('./plots/'+name+'.png')
    plt.clf()
    #plt.show()

    # # #### SECOND PARAMETER(Uniformity) ######
    ...
    y_cord = np.ravel(y_cord)
    y_cord.sort()
    dist = []

    for i in range(y_cord.shape[0]):
     if(i+1<y_cord.shape[0]):
         d = y_cord[i+1] - y_cord[i]
         dist.append(d)

    dist = minmax(dist)
    dist_sd = np.std(dist,axis = 0)

    uniformity.append(dist_sd)
    print("Parameter 2 score(Uniformity): ",dist_sd)

print("len of alignment", len(alignment))
print("len of uniformity", len(uniformity))


alignment = np.array(alignment)
uniformity = np.array(uniformity)

np.save("alignment", alignment)
np.save("uniformity", uniformity)
